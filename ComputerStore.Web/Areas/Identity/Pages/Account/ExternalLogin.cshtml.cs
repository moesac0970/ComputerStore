...
namespace ComputerStore.Web.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        private readonly ClientBearerService bearerService;
        ...
        public ExternalLoginModel(
            ...
            ClientBearerService clientBearerService)
        {
            baseUri = configuration.GetSection("Data").GetSection("ApiBaseUri").Value;
            bearerService = clientBearerService;
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        // add bearer token when login via external login provider

                        await _signInManager.SignInAsync(user, isPersistent: false);
                        _logger.LogInformation("User created an account using {Name} provider.", info.LoginProvider);

                        var bearerToken = await bearerService.SendBearerRequest(baseUri, Input.Email);
